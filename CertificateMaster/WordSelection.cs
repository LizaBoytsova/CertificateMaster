...
namespace CertificateMaster
{
    ...
    class WordSelection
    {
        ...
        public bool Underline
        {
            ...
            {
                // открытый вопрос с возможными занчениями в Word по умолчанию, документация плохая
                if (_range.Underline == Word.WdUnderline.wdUnderlineSingle) { return true; }
                else { return false; }
            }
            set
            {
                if (value == true) { _range.Underline = Word.WdUnderline.wdUnderlineSingle; }
                else { _range.Underline = Word.WdUnderline.wdUnderlineNone; }
            }
            // завершение  public bool Italic
        }
